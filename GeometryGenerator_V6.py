...
MIN_FEATURE_SIZE = 0.0005 # 0.5 mm Minimum Feature Size (Fluid or Solid)
...
# =============================================================================
# USER OPTION: STRAIGHT vs RANDOM
# =============================================================================
print("\nSelect Geometry Mode:")
print("1: Random / Manhattan Routing (Default)")
print("2: Straight Channels")
print("3: Uniform Value (0.0 - 1.0)")
user_choice = input("Enter choice (1, 2, or 3): ").strip()

USE_STRAIGHT = (user_choice == '2')
USE_UNIFORM_VAL = (user_choice == '3')
UNIFORM_VAL = 1.0

if USE_STRAIGHT:
    print(">> MODE SELECTED: STRAIGHT CHANNELS")
elif USE_UNIFORM_VAL:
    try:
        val_str = input("Enter uniform value (0.0 - 1.0): ").strip()
        UNIFORM_VAL = float(val_str)
        UNIFORM_VAL = max(0.0, min(1.0, UNIFORM_VAL))
    except ValueError:
        print("Invalid input, defaulting to 0.5")
        UNIFORM_VAL = 0.5
    print(f">> MODE SELECTED: UNIFORM VALUE = {UNIFORM_VAL}")
else:
    print(">> MODE SELECTED: RANDOM / MANHATTAN")
...
w_channel_min = max(MIN_FEATURE_SIZE, 3.5 * dx)
...
# Skip rest of generation if uniform value
if not USE_UNIFORM_VAL:
    # --- A. Create Inlet Plenum (Fully Open) ---
    if N_inlet_plenum > 0:
        geometry[:, 0:N_inlet_plenum] = 0.0
...
if USE_STRAIGHT:
    SPLIT_PROB = 0.0          # No splitting
    WANDER_INTENSITY = 0.0    # No wandering
    MERGE_OVERLAP_FACTOR = 1.0 # Harder to merge (or just disabled in loop)
    TURN_MAGNITUDE = 0.0
else:
    SPLIT_PROB = 0.2        # Probability to split if width allows
    WANDER_INTENSITY = 0.4   # Standard deviation of y-perturbation per step (in routing units)
    MERGE_OVERLAP_FACTOR = 0.5 # Fraction of sum-of-widths to trigger merge
...
if not USE_UNIFORM_VAL:
    for x_r in range(x_hs_start_r + 1, x_hs_end_r + 1):
        ...
        # C. Merge Logic
        # Disable merging for straight channels to keep them parallel
        if not USE_STRAIGHT:
...
# --- 4.5 ROBUST MORPHOLOGICAL CLEANUP (SCIPY) ---

def cleanup_topology(geom, dx_val):
    if USE_UNIFORM_VAL:
        return geom
        
    # 1. Enforce Minimum Solid Width (Opening on Solids)
    # Any solid feature cannot be smaller than MIN_FEATURE_SIZE
    
    # Calculate kernel size (in pixels)
    # We want to remove solids thinner than min_width.
    # Opening with a kernel of size K essentially keeps only things where K fits.
    
    min_pixels = int(np.ceil(MIN_FEATURE_SIZE / dx_val))
    print(f"Morphological Cleanup: Minimum Feature = {MIN_FEATURE_SIZE*1000} mm -> {min_pixels} pixels")
    
    # Create structure element (Square for Manhattan corners)
    # Using a fully connected square (1s) of size (min_pixels x min_pixels)
    # This preserves sharp corners while ensuring no feature is thinner than min_pixels.
    
    structure = np.ones((min_pixels, min_pixels), dtype=int)
    
    # Cast input to boolean (Solid is 1)
    # For float geometry: threshold at 0.5 or treat as binary for cleanup?
    # Original logic assumes binary 0/1. If we have uniform floats, this function is skipped anyway.
    solid_mask = (geom > 0.5)
    
    # Apply Binary Opening to Solids
    # This erodes (removes thin stuff) then dilates (restores bulk)
    cleaned_solid = scipy.ndimage.binary_opening(solid_mask, structure=structure)
    
    # Reconstruct geometry
    new_geom = np.zeros_like(geom, dtype=float)
    new_geom[cleaned_solid] = 1.0
    
    # 2. OPTIONAL: Enforce Minimum Fluid Width (Opening on Fluid / Closing on Solid)
    # If the user also wants to ensure no tiny fluid gaps, we would do opening on fluid.
    # The prompt specifically emphasized "material is at least...".
    # However, "no super small features... at the edge of a channel" implies fluid too.
    # Let's apply valid fluid cleanup too to be safe.
    
    fluid_mask = (new_geom == 0.0)
    cleaned_fluid = scipy.ndimage.binary_opening(fluid_mask, structure=structure)
    
    # Where fluid was removed (cleaned), it becomes solid?
    # This fills in tiny gaps.
    # Re-apply to geometry: if it's in cleaned_fluid, it's 0. Else 1.
    
    final_geom = np.ones_like(geom, dtype=float) # Default solid
    final_geom[cleaned_fluid] = 0.0           # Set confirmed fluid
    
    # Calculate how many pixels changed
    diff = np.sum(geom != final_geom)
    print(f"Cleanup finished. {diff} pixels changed.")
    
    return final_geom

# Apply cleanup
geometry = cleanup_topology(geometry, dx)

# Force top/bottom boundary again just in case cleanup ate the edges
if not USE_UNIFORM_VAL:
    geometry[0, :] = 1.0
    geometry[-1, :] = 1.0
...
np.savetxt(os.path.join(OUTPUT_DIR, "geometry_fluid.txt"), geometry, fmt='%.6f', delimiter='\t')
...
plt.title(
    f"File: geometry_fluid.txt | Refine {REFINEMENT_FACTOR}x | {num_channels} Ch\n"
    f"Seed: {RANDOM_SEED} | Min Feature: {MIN_FEATURE_SIZE*1000}mm"
)
plt.xlabel("X (Cells)")
plt.ylabel("Y (Cells)")
plt.axvline(x=N_inlet_plenum, color='r', linestyle='--', label='Plenum End')
plt.axvline(x=N_inlet, color='g', linestyle='--', label='Heatsink Start')
plt.axvline(x=N_inlet + N_heatsink, color='b', linestyle='--', label='Heatsink End')
